package CodeForces.Hashing.A_Vitaly_And_Pie;
...
public class Main {
      public static char getEquvialent(char ch) {
            return Character.toLowerCase(ch);
      }

      public static int getminBuys(String s, int n) {
            int buys = 0;

            int[] frq = new int[26];

            for (int i = 0; i < (2 * n) - 2; i += 2) {
                  char door = s.charAt(i + 1);
                  char key = s.charAt(i);

                  char required = getEquvialent(door);

                  if (required != key) {
                        if (frq[required - 'a'] == 0) {
                              buys++;
                        } else if (frq[required - 'a'] > 0)
                              frq[required - 'a']--;
      ...
      public static void main(String[] args) {
            ...
            System.out.println(getminBuys(s, n));
