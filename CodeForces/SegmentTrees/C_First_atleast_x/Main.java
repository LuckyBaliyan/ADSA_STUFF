package CodeForces.SegmentTrees.C_First_atleast_x;
...
public class Main {
    ...
    public static void update_St(int i,long v,int idx,int l,int r){
        ...
        if(i<=mid)update_St(i, v, 2*idx, l, mid);
        else update_St(i, v, 2*idx+1, mid + 1, r);

        seg[idx] = Math.max(seg[2*idx], seg[2*idx+1]);
    }

    public static int giveX(long x,int idx,int l,int r){
        ...
        if(seg[2*idx] >= x)return giveX(x, 2*idx, l, mid);
        else return giveX(x, 2*idx+1, mid + 1, r);
    }
